...
export default function ArticlesPractice({ questions, onComplete }: Props) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const keyMap: Record<string, string> = { '1': 'a', '2': 'an', '3': 'the', '4': 'at', '5': '–' }
      if (keyMap[e.key]) handleChoice(keyMap[e.key])
    }
    window.addEventListener('keydown', handleKeyDown)
    return () => window.removeEventListener('keydown', handleKeyDown)
  }, [currentIndex, isCorrect, showExplanation, cooldown, localQuestions.length])
        ...
        {localQuestions.map((q, index) => {
          ...
          return (
            <Card
              ...
              onClick={() => { if (!isDone) setCurrentIndex(index) }}>
              <CardBody className="p-6 sm:p-8">
                <div className="space-y-3">
                  ...
                  <p className="text-default-400 font-bold uppercase tracking-widest text-[10px] italic">{q.pl}</p>
                </div>
              </CardBody>
            </Card>
          )
          ...
          <div className="grid grid-cols-5 gap-2 sm:gap-3">
            {OPTIONS.map((opt, i) => (
              <GameButton
                ...
                color={currentIndex < localQuestions.length && selectedOption === opt ? (isCorrect ? 'success' : 'danger') : 'primary'}
                isDisabled={isCorrect === true || showExplanation}
                variant={currentIndex < localQuestions.length && selectedOption === opt ? 'solid' : 'bordered'}
                ...
                <Card className="border-none bg-white border-2 border-danger/20 rounded-2xl shadow-xl">
                  <CardBody className="p-5 text-center flex flex-row items-center justify-between gap-4">
                    <div className="text-left">
                      ...
                      <p className="text-xs sm:text-sm font-bold text-danger/80 leading-tight italic">{currentQ.explanation}</p>
                    </div>
                    <GameButton
                      ...
                      <div className="absolute left-0 bottom-0 top-0 bg-white/20 transition-all duration-75" style={{ width: `${cooldown}%` }} />
