...
export const WordGrid: React.FC<WordGridProps> = React.memo(({ grid, onItemClick }) => {
  return (
    <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5 sm:gap-2.5 w-full flex-1 max-w-2xl mx-auto px-1 sm:px-0 auto-rows-fr">
      {grid.map((item, index) => (
        <div key={`cell_${index}`} className="relative w-full aspect-square">
          <AnimatePresence mode="wait">
            ...
            >
              <Card 
                isPressable 
                className="w-full h-full bg-white transition-all border-1 sm:border-2 border-transparent hover:border-primary shadow-sm hover:shadow-xl group overflow-hidden relative rounded-xl sm:rounded-[1.5rem]"
                onClick={(e) => {
                  e.preventDefault();
